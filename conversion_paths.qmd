...
transition_matrix |> 
  ...
  #scale_fill_brewer(palette = RColorBrewer::brewer.pal(11, "BrBG")) +
  scale_fill_gradient(
    low = "tomato2",
    high = "olivedrab3",
    na.value = "cornsilk4"
...
-   To end up in the `(conversion)` state, the highest transition probability 82.6% is coming from `(none`). Compare this with 68.8% coming from `referral`
...
-   also conversely, crossing a `cpc` paid search touchpoints is most likely coming from an `organic` search
...
-   `(none`) is getting under-valued in the country/medium view, being credited there with only like \~8% of the revenue. In this view it is almost on par actually with `referral`
