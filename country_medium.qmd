...
```{r}
#| label: high-vol-free-traffic-wide
countries_rank_by_low_free_sessions <- free_vs_paid_by_country |> 
  ...
  # and rank them w.r.t to the % of free sessions traffic
  mutate(
...
```{r}
#| label: high-vol-free-traffic-plot
countries_rank_by_low_free_sessions |> 
  ...
  scale_x_continuous(breaks = seq(0, 0.25, 0.01), labels = label_percent(accuracy = 1)) +
...
```{r}
#| label: high-vol-free-traffic-table
countries_rank_by_low_free_sessions |> 
  filter(country != "(not set)") |> 
  mutate(
    is_on_boundary = (perc_sessions_free < 0.85) & (mean_conv_rate_free > 0.022)
  ) |> 
  filter(is_on_boundary) |> 
  datatable()
```
:::
...
free_vs_paid_by_country |> 
  ...
  # need to make one of the conv.rates negative in order
  # to plot them from-center-to-the-sides like a pyramid
  mutate(
  ...
  # remove not set as it's not actionable here
  filter(country != "(not set)") |> 
  # add the extra columns
  left_join(countries_rank_by_low_free_sessions, join_by(country)) |> 
  mutate(diff_conv_rates = mean_conv_rate_free - mean_conv_rate_paid) |> 
  select(country, medium, mean_conv_rate, perc_sessions_free, diff_conv_rates) |>
  mutate(country = fct_reorder(country, diff_conv_rates, max))
```
...
free_vs_paid_by_country |> 
  ...
  #filter(perc_sessions_free_rank <= 40) |> 
  
  mutate(diff_conv_rates = mean_conv_rate_free - mean_conv_rate_paid) |> 
  select(country, medium, mean_conv_rate, diff_conv_rates) |>
  mutate(country = fct_reorder(country, diff_conv_rates)) |>

  # order the countries by ther % free sessions
  # the countries on the top have the highest % of free sessions overall
  ...
  facet_grid(~ medium, scales = "free_x")
```
...
free_vs_paid_by_country |> 
  ...
  geom_segment(aes(x = 0, xend = diff_conv_rates)) +
  scale_x_continuous(
